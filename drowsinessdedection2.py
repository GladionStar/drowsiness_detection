...
def calculate_blinks_per_minute():
...
def main():
    global drowsy_frames, alert_active, last_alert_time, blink_start, blink_frames, total_blinks, blink_times, start_time
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                blinks_per_minute = calculate_blinks_per_minute()
                cv2.putText(frame, f"Blinks/min: {blinks_per_minute}", (10, 60), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                ...
                if ear < BLINK_THRESHOLD and not blink_start:
                    blink_start = True
                elif ear < BLINK_THRESHOLD and blink_start:
                    blink_frames += 1
                elif ear >= BLINK_THRESHOLD and blink_start:
                    if blink_frames > 1:
                        total_blinks += 1
