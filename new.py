...
MIN_BLINK_RATE = 10
MAX_BLINK_RATE = 30
MOUTH = [61, 291, 78, 308, 14, 13, 312, 82, 87, 178, 88, 95]
BLINK_COOLDOWN = 0.4
...
class ModernDrowsinessDetection:
    def __init__(self):
        ...
        self.yawn_times = deque(maxlen=60)
        self.yawn_count = 0
    ...
    # Adjust MAR calculation to use correct indices and handle incomplete data
    def calculate_mar(self, mouth_landmarks):
        try:
            if len(mouth_landmarks) < 12:  # Ensure there are enough points
                return 0

            # Correct indices for top and bottom lips
            top_lip = (mouth_landmarks[13][1] + mouth_landmarks[14][1]) / 2
            bottom_lip = (mouth_landmarks[87][1] + mouth_landmarks[178][1]) / 2

            # Calculate mouth height and width
            mouth_height = abs(top_lip - bottom_lip)
            left_corner = mouth_landmarks[61][0]
            right_corner = mouth_landmarks[291][0]
    ...
    def process_frame(self, frame):
        ...
        if results.multi_face_landmarks:
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                self.mar = self.calculate_mar(mouth_coords)

                current_time = time.time()
                if not self.yawn_in_progress and self.mar > MAR_THRESHOLD:
                    self.yawn_frames += 1
                    if self.yawn_frames >= YAWN_FRAMES_THRESHOLD:
                        self.yawn_in_progress = True
                        self.yawn_start_time = current_time
                elif self.yawn_in_progress and self.mar <= MAR_THRESHOLD:
                    if current_time - self.yawn_start_time > 1.0:
                        self.yawn_times.append(current_time)
                        self.yawn_count += 1
                    self.yawn_in_progress = False
                    self.yawn_frames = 0

                # Ek yorgunluk analizi
                drowsy_by_yawn = len([t for t in self.yawn_times if time.time() - t < 300]) >= YAWN_ALERT_THRESHOLD
                drowsy_by_blink_rate = (time.time() - self.session_start_time) > 30 and self.calculate_blinks_per_minute() < MIN_BLINK_RATE

                if drowsy_by_yawn or drowsy_by_blink_rate:
                    cv2.putText(frame, "YORGUNLUK ALARMI!", (10, 60), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    if not self.alert_active or (time.time() - self.last_alert_time) > ALERT_COOLDOWN:
                        alert_sound.play()
                ...
                # Updated process_frame to include yawn detection and blink tracking
                for idx in MOUTH:
                    landmark = face_landmarks.landmark[idx]
                    x, y = int(landmark.x * w), int(landmark.y * h)
                    mouth_coords.append((x, y))
                ...
                yawns_in_period = len([t for t in self.yawn_times if time.time() - t < 300])
                if yawns_in_period >= YAWN_ALERT_THRESHOLD:
                    cv2.putText(frame, "YORGUNLUK ALARMI! Cok fazla esneme", (10, 150), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    if not self.alert_active or (current_time - self.last_alert_time) > ALERT_COOLDOWN:
                        alert_sound.play()
                        self.alert_active = True
                ...
                if not self.blink_start and self.current_ear < BLINK_THRESHOLD:
                    self.blink_start = True
                    self.blink_start_time = current_time
                elif self.blink_start and self.current_ear >= BLINK_THRESHOLD:
                    blink_duration = current_time - self.blink_start_time
                    self.blink_start = False
                    if blink_duration < 0.5 and (current_time - self.last_blink_time) > BLINK_COOLDOWN:
    ...
    def draw_stats_panel(self):
        ...
        y_pos += 25
        mar_label = self.normal_font.render(f"MAR: {self.mar:.2f}", True, LIGHT_GRAY)
        self.screen.blit(mar_label, (SCREEN_WIDTH - PANEL_WIDTH + 20, y_pos))
        
        y_pos += 25
        yawn_label = self.normal_font.render(f"Toplam Esneme: {self.yawn_count}", True, LIGHT_GRAY)
        self.screen.blit(yawn_label, (SCREEN_WIDTH - PANEL_WIDTH + 20, y_pos))
